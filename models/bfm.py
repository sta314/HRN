...
class ParametricFaceModel:
    def __init__(self, 
        ...
        model_bfm_front = loadmat(os.path.join(bfm_folder, 'BFM_model_front.mat'))
    ...
    def compute_albedo_map(self, tex_coeff, normalize=True):
        batch_size = tex_coeff.shape[0]
        face_texture = torch.einsum('ij,aj->ai', self.tex_base_uv, tex_coeff) + self.mean_tex_uv
        if normalize:
            face_texture = face_texture / 255.
    ...
    def compute_color_map(self, face_texture_uv, face_norm_uv, gamma):
        ...
        Y = torch.cat([
            ...
            0.5 * a[2] * c[2] / np.sqrt(3.) * (3 * face_norm_uv[..., 2:] ** 2 - 1),
        ...
        g = Y @ gamma[..., 1:2]
        b = Y @ gamma[..., 2:]
        face_color = torch.cat([r, g, b], dim=-1).reshape(batch_size, size, size, 3) * face_texture_uv  # (B, 256, 256, 3)
    ...
    def displacement2normal(self, uv_z, coarse_verts, coarse_normals):
        ...
        uv_detail_normals = deca_util.vertex_normals(dense_vertices, self.render.dense_faces.expand(batch_size, -1, -1))
        ...
        dense_vertices = uv_detail_vertices.permute(0, 2, 3, 1).reshape([batch_size, -1, 3])
        uv_detail_normals = deca_util.vertex_normals(dense_vertices, self.render.dense_faces.expand(batch_size, -1, -1))
        # uv_detail_normals = self.compute_norm()
        uv_detail_normals = uv_detail_normals.reshape(
            [batch_size, uv_coarse_vertices.shape[2], uv_coarse_vertices.shape[3], 3]).permute(0, 3, 1, 2)
        uv_detail_normals[:, :2, ...] = -uv_detail_normals[:, :2, ...]
    ...
    def get_texture_map(self, face_vertex, input_img):
        ...
        texture_map = texture_map.reshape(batch_size, self.render.uv_size, self.render.uv_size, -1)  # (B, 256, 256, 3)
    ...
    def compute_for_render_hierarchical_mid(self, coeffs, deformation_map, UVs, visualize=False, de_retouched_albedo_map=None):
        ...
        face_shape = self.compute_shape(coef_dict['id'], coef_dict['exp'])  # (B, n, 3)
        face_shape_base = face_shape.clone()

        face_shape, shape_offset = self.add_nonlinear_offset(face_shape, deformation_map, UVs)  # (B, n, 3)
        ...
        # face_texture = self.compute_albedo(coef_dict['tex'])
        face_albedo_map = self.compute_albedo_map(coef_dict['tex'])  # (B, 512, 512, 3)
        face_albedo_map = face_albedo_map.permute(0, 3, 1, 2)
        face_albedo_map = torch.nn.functional.interpolate(face_albedo_map, [self.render.uv_size, self.render.uv_size], mode='bilinear')
        face_norm_roted_uv = self.render.world2uv(face_norm_roted)
    ...
    def add_nonlinear_offset(self, face_shape, shape_offset_uv, UVs):
        ...
        shape_lt = shape_offset_uv[:, (h - 1 - UVs_coords_int[:, 1]).clamp(0, h-1), UVs_coords_int[:, 0].clamp(0, w-1)]  # (B, N, 3)
        shape_lb = shape_offset_uv[:, (h - UVs_coords_int[:, 1]).clamp(0, h-1), UVs_coords_int[:, 0].clamp(0, w-1)]
        shape_rt = shape_offset_uv[:, (h - 1 - UVs_coords_int[:, 1]).clamp(0, h-1), (UVs_coords_int[:, 0] + 1).clamp(0, w-1)]
        shape_rb = shape_offset_uv[:, (h - UVs_coords_int[:, 1]).clamp(0, h-1), (UVs_coords_int[:, 0] + 1).clamp(0, w-1)]

        offset_shape = shape_lt * (1 - UVs_coords_float[:, :1]) * UVs_coords_float[:, 1:] + \
                       shape_lb * (1 - UVs_coords_float[:, :1]) * (1 - UVs_coords_float[:, 1:]) + \
                       shape_rt * UVs_coords_float[:, :1] * UVs_coords_float[:, 1:] + \
                       shape_rb * UVs_coords_float[:, :1] * (1 - UVs_coords_float[:, 1:])  # (B, N, 3)
